...
export const FindXptablesInput = z
  .object({
  ...
  .refine(requireAtLeastOneFilter('internal_name', 'internal_name_contains'), {
    message: 'find_xptables requires at least one filter field',
  });

export type FindXptablesArgs = z.infer<typeof FindXptablesInput>;

export async function runFindXptables(args: FindXptablesArgs, manager: SourceManager) {
  const t0 = performance.now();
  const loaded = await surfaceSourceError(() => manager.load('xptables'));
...
function matchesFilters(entry: Entry, args: FindXptablesArgs): boolean {
